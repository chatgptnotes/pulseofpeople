-- ============================================================================
-- ULTRA SAFE MIGRATION - Works regardless of current database state
-- ============================================================================
-- This migration carefully checks and fixes everything step by step
...
-- ============================================================================
-- STEP 1: Fix organizations table - Add columns one by one
-- ============================================================================

DO $$
BEGIN
    -- Add each column individually with careful checks
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='organizations' AND column_name='type') THEN
        ALTER TABLE organizations ADD COLUMN type VARCHAR(50) DEFAULT 'political_party';
    ...
    RAISE NOTICE 'All organization columns added successfully';
END $$;

-- ============================================================================
-- STEP 2: Generate slugs for existing organizations (now that column exists)
-- ============================================================================

DO $$
BEGIN
    -- Only update if slug column exists and has NULL values
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='organizations' AND column_name='slug') THEN
        UPDATE organizations
        SET slug = LOWER(REGEXP_REPLACE(name, '[^a-zA-Z0-9]+', '-', 'g'))
        WHERE slug IS NULL;

        RAISE NOTICE 'Generated slugs for existing organizations';
    END IF;
END $$;

-- ============================================================================
-- STEP 3: Add constraints (now that columns exist and have values)
-- ============================================================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='organizations' AND column_name='slug') THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not set slug to NOT NULL - may have NULL values';
        END;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'organizations_slug_key') THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not add unique constraint on slug - may have duplicates';
        END;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'organizations_type_check') THEN
        BEGIN
            ALTER TABLE organizations ADD CONSTRAINT organizations_type_check
                CHECK (type IN ('political_party', 'campaign', 'ngo', 'advocacy_group'));
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not add type constraint';
        END;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'organizations_subscription_status_check') THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not add subscription_status constraint';
        END;
    END IF;
END $$;
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Don't fail user creation if audit log fails
    END;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    IF creator_role = 'superadmin' OR creator_role IS NULL THEN RETURN NEW; END IF;
